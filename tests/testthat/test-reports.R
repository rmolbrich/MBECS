...
test_that("mbecReportPrelim works", {
    ...
    PreRep.test <- evaluate_promise(
        mbecReportPrelim(input.obj=dummy.mbec, model.vars=c("batch","group"),
    ...
    # mosaic works
    ds.test <- mbecMosaic(
        input.obj=dummy.mbec, model.vars=c("batch","group"), return.data=TRUE)
    ...
    # box works
    ds.test <- mbecBox(
        input.obj=dummy.mbec, method="TOP", n=5, model.var="batch",
        return.data=TRUE)
    ...
    # linmixmod works
    ds.test <- mbecModelVariance(
        input.obj=dummy.mbec, model.vars=c("batch","group"), method="lmm",
        type="clr")

    expect_equal(PreRep.test$result$linmixmod, ds.test)
    # rda works
    ds.test <- mbecModelVariance(
        input.obj=dummy.mbec, model.vars=c("batch","group"), method="rda",
        type="clr")

    expect_equal(PreRep.test$result$rda, ds.test)
    # pvca works
    ds.test <- mbecModelVariance(
        input.obj=dummy.mbec, model.vars=c("batch","group"), method="pvca",
        type="clr")

    expect_equal(PreRep.test$result$pvca, ds.test)
    # scoef works
    ds.test <- mbecModelVariance(
        input.obj=dummy.mbec, model.vars=c("batch","group"), method="s.coef",
        type="clr")
    ...
    PreRep.test <- evaluate_promise(
        mbecReportPrelim(input.obj=dummy.list, model.vars=c("batch","group"),
                         type="clr", file.name=NULL, file.dir=NULL,
                         return.data=TRUE))
...
test_that("mbecReportPost works", {
    ...
    expect_error(
        mbecReportPost(input.obj=dummy.mbec, model.vars=c("batch","group"),
                       type="clr", file.name=NULL, file.dir=NULL,
                       return.data = TRUE), "No corrections available.")
    # Apply ComBat to the dummy
    dummy.corrected <- mbecCorrection(input.obj=dummy.mbec,
