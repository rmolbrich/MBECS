...
mbecReportPrelim <- function(input.obj, model.vars=c("group","batch"), return.plot = FALSE) {
  ...
  # but prelim can be called with mbecGetData and then it will only be a list with cnts and meta anyway
  ...
  prelim.report.list[["linmod"]] <- mbecModelVariance(input.obj, method="lm", 
