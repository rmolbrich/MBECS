...
#' @examples
#' # This will return a matrix of normalised counts, according to the covariate information in meta
#' \dontrun{plot <- ggplot2::boxplot(data) + theme_poster(base_size = 18, base_family = "Myriad Pro")}
theme_poster <- function (base_size = 18, base_family = "Myriad Pro") {
  theme(
    line = element_line(colour = "#7F7F7F", size = 0.5,
    ...
    axis.text.x=element_text(angle=35, hjust=1, color = eval(main_color), size=18),
    ...
    strip.text.x = element_text(size = base_size, color = eval(main_color), face = "bold"),
    strip.text.y = element_text(size = base_size, color = eval(main_color), face = "bold"),
    strip.background = element_rect(color=eval(main_color), fill="transparent", size=1.5, linetype="solid"),

    legend.position="bottom",
    legend.text = element_text(color = eval(main_color), size=18),
    ...
    ### ALTERNATIV ###
    panel.grid.major = element_line(colour = "#7F7F7F", size = 0.25),
...
#' @examples
#' # This will return a matrix of normalised counts, according to the covariate information in meta
#' \dontrun{plot <- mbecRLE(input.obj, model.vars=c("group","batch"), return.data=FALSE) +
#' theme_rle(base_size = 18, base_family = "Myriad Pro",legend_position = 'bottom')}
theme_rle <- function(base_size = 18, base_family = "Myriad Pro",legend_position = 'bottom') {
  theme_bw() +
    # adjustments for the legend
    theme(legend.position=eval(legend_position),
          legend.text = element_text(color = "black", size=12),
          legend.key = element_rect(size=12),
          axis.text.x=element_text(angle=35, hjust=1),
          axis.title.x = element_blank(),
...
theme_box <- function(base_size = 18, base_family = "Myriad Pro",legend_position = 'bottom') {
  ...
  theme_bw() +
    theme(
      ...
      axis.line = element_blank(),
      axis.ticks = element_blank(),
      axis.text.x = element_text(angle = x.angle, hjust = x.hjust), panel.grid = element_blank(),
