...
#' DEPRECATED
#' Helper function that fits lm/lmm with covariates 'treatment' and 'batch' to every feature in the dataset
...
#' @export
mbecLM <- function(input.obj, method=c("auto","lm","lmm"), grouping=c("batch","group")) {
  ...
  tmp <- mbecGetData(input.obj, orientation="sxf", required.col=eval(grouping))
  ...
  if( method == "auto" ) {
    ...
    tmp.group.p <- apply(tmp.cnts, 2, FUN = function(x){
      nc.lm <- stats::lm(x ~ group + batch, data = tmp.meta)
    ...
    tmp.group.p <- apply(tmp.cnts, 2, FUN = function(x){
      nc.lmm <- lmerTest::lmer(x ~ group + (1|batch), data = tmp.meta)
...
#' Wrapper to help perform log-ratio transformations.
#' Adapted from packages 'mixOmics' and 'robCompositions' to work on matrices and Phyloseq objects alike.
...
#' The function applies normalization to every batch of a case/control study, is called by 'batchCorrection()'
#' group/treatment vector is limited to two classes, i.e., case/control study
#' @param cnts, matrix of cnts in sxf orientation
#' @param meta, df that requires columns 'group' and 'batch'
#' @export
percentileNorm <- function(cnts, meta) {
  
  ref.group <- levels(meta$group)[1]
  ...
  # for every batch
  for( b.idx in levels(meta$batch) ) {
    # for every feature
    for( f.idx in 1:ncol(cnts) ) {
      # which are the control-group values
      ctrl.group.vec <- cnts[which((meta$group %in% ref.group) & (meta$batch %in% b.idx)), f.idx]
      # for every sample in the batch
      for( s.idx in which(meta$batch %in% b.idx) ) {
...
#' Implementation of Claire Duvallet's 'percentileofscore()' for python
#' Will return values in the interval [0,100] and 100 if reference vector has zero length.
