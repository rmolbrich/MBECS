...
```{r ACTIVATION, echo=TRUE, eval=TRUE, results='asis', tidy=TRUE}
library(MBECS)
```
...
-   Percentile Normalization (pn)
    ...
    - <span style="color: orange;">The 'mbecRunCorrections' wrapper will automatically select Total Sum Scaled values for percentile normalization because that is what it is supposed to be used on. The function 'mbecCorrections' can be used to manually select untransformed or centered log ratio transformed abundances.</span>
...
The user has the choice between two functions 'mbecCorrection' and 'mbecRunCorrections', the latter one acts as a wrapper that can apply multiple correction methods in a single run. <span style="color: orange;">If the input to 'mbecRunCorrections' is missing CLR and TSS transformed abundance matrices, they will be created with default settings, i.e., offsets for zero values will be set to 1/#features.</span>
...
```{r Usage_rle, echo=TRUE, eval=TRUE, results='asis', tidy=TRUE}
# Create an MbecData object from your data-set.
mbec.obj <- mbecProcessInput(dummy.list)

# Apply CLR transformtion with offset of 0.0001
mbec.obj <- mbecTransform(mbec.obj, method = "clr", offset = 0.0001)
...
rle.plot <- mbecRLE(input.obj=mbec.obj, model.vars = c("batch","group"), type="clr",
                    return.data = FALSE) 
```
...
```{r Usage_pca, echo=TRUE, eval=TRUE, results='asis', tidy=TRUE}
...
pca.plot <- mbecPCA(input.obj=mbec.obj, model.vars = c("batch","group"), type="clr",
                    pca.axes=c(1,2), return.data = FALSE) 

# It is possible to select only a single covariate for plotting.
...
```{r Usage_box, echo=TRUE, eval=TRUE, results='asis', tidy=TRUE}
# Produce Box-plots of the most variable features on clr transformed values. The 'method' parameter determines if 'ALL' or only the 'TOP' n features are plotted. The 'n' parameter selects the number of features to plot. The function will return a list of plot object to be used.
...
Pheatmap is used to produce heatmaps of the most variable features with the 'mbecHeat' function.

```{r Usage_heat, echo=TRUE, eval=TRUE, results='asis', tidy=TRUE}
...
```{r Usage_mosaic, echo=TRUE, eval=TRUE, results='asis', tidy=TRUE}
# Produce a mosaic plot that shows how the samples are distrubuted over two covariates of interest.
mosaic.plot <- mbecMosaic(input.obj=mbec.obj, model.vars = c("batch", "group"),
                          return.data = FALSE)
```
...
Fit a linear model of the form 'y \~ group + batch' to every feature and show the proportion of variance with regards to the covariates in a box-plot.

```{r Usage_varLM, echo=TRUE, eval=TRUE, results='hide', fig.keep='all', message = FALSE, warning = FALSE, tidy=TRUE}
# Apply SVD batch correction to CLR transformed abundances, to evaluate the results.
mbec.obj <- mbecCorrection(mbec.obj, model.vars=c("batch","group"), 
                           method = "svd", type = "clr")

# Calculate proportion of variance with regards to covariates of interest for every feature.
lm.variance <- mbecModelVariance(input.obj=mbec.obj, model.vars = c("batch", "group"),
...
Fit a linear mixed model of the form 'y \~ group + (1\|batch)' to every feature and show the proportion of variance with regards to the covariates in a box-plot.

```{r Usage_varLMM, echo=TRUE, eval=TRUE, results='hide', fig.keep='all', message = FALSE, warning = FALSE, tidy=TRUE}
# Apply SVD batch correction to CLR transformed abundances, to evaluate the results.
mbec.obj <- mbecCorrection(mbec.obj, model.vars=c("batch","group"), 
                           method = "ruv3", type = "clr")

# Calculate proportion of variance with regards to covariates of interest for every feature.
lmm.variance <- mbecModelVariance(input.obj=mbec.obj, model.vars = c("batch","group"),
                                 method="lmm",type="cor", label="ruv3")
...
Use partial Redundancy Analysis to calculate the variance that can be attributed to the covariates of interest.

```{r Usage_varRDA, echo=TRUE, eval=TRUE, results='hide', fig.keep='all', message = FALSE, warning = FALSE, tidy=TRUE}
# Apply ComBat to CLR transformed abundances, to evaluate the results.
mbec.obj <- mbecCorrection(mbec.obj, model.vars=c("batch","group"), 
                           method = "bat", type = "clr")

# Calculate percentage of variance that can be attributed to the covariates.
rda.variance <- mbecModelVariance(input.obj=mbec.obj, model.vars = c("batch", "group"),
...
Use PVCA to calculate the variance that can be attributed to the covariates of interest.

```{r Usage_varPVCA, echo=TRUE, eval=TRUE, results='hide', fig.keep='all', message = FALSE, warning = FALSE, tidy=TRUE}
# Apply RemoveBatchEffect to CLR transformed abundances, to evaluate the results.
mbec.obj <- mbecCorrection(mbec.obj, model.vars=c("batch","group"), 
                           method = "rbe", type = "clr")

# Calculate percentage of variance that can be attributed to the covariates.
pvca.variance <- mbecModelVariance(input.obj=mbec.obj, model.vars = c("batch", "group"),
...
```{r Usage_varSCOEF, echo=TRUE, eval=TRUE, results='hide', fig.keep='all', message = FALSE, warning = FALSE, tidy=TRUE}
# Apply BatchMeanCentering to CLR transformed abundances, to evaluate the results.
mbec.obj <- mbecCorrection(mbec.obj, model.vars=c("batch","group"), 
                           method = "bmc", type = "clr")

# Calculate the Silhouette Coefficient scores to evaluate how good the data fits the clustering implied by covariates and plot results.
sil.coefficient <- mbecModelVariance(input.obj=mbec.obj, model.vars = c("batch", "group"),
